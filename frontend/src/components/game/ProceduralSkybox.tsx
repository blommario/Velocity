...
export function ProceduralSkybox({ type = 'day', radius = 500 }: ProceduralSkyboxProps) {
  ...
  useEffect(() => {
    const preset = SKY_PRESETS[type];

    // Pre-normalize sun direction on CPU
    const sx = preset.sunPos[0], sy = preset.sunPos[1], sz = preset.sunPos[2];
    const len = Math.sqrt(sx * sx + sy * sy + sz * sz);

    const sunDirection = uniform(vec3(sx / len, sy / len, sz / len));
    const uSunColor = uniform(vec3(preset.sunColor[0], preset.sunColor[1], preset.sunColor[2]));
    const uZenithColor = uniform(vec3(preset.zenithColor[0], preset.zenithColor[1], preset.zenithColor[2]));
    const uHorizonColor = uniform(vec3(preset.horizonColor[0], preset.horizonColor[1], preset.horizonColor[2]));
    const sunSize = uniform(float(preset.sunSize));
    const sunIntensity = uniform(float(preset.sunIntensity));
    ...
    // colorNode with lights=false should produce unlit output
